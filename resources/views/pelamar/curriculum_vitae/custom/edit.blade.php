...
<head>
  <meta charset="UTF-8">
  <title>Edit Custom Section | CVRE GENERATE</title>
...
<div class="max-w-3xl mx-auto py-10">
  ...
  <h1 class="text-2xl font-semibold text-blue-800 mb-6">Edit Custom Section</h1>

  <form method="POST" action="{{ route('pelamar.curriculum_vitae.custom.update', [$curriculumVitaeUser->id, $customSection->id]) }}">
    @csrf @method('PUT')
    ...
    <div class="bg-white p-5 rounded border shadow-sm space-y-4">
      <div>
        ...
        <input type="text" name="section_title" value="{{ old('section_title', $customSection->section_title ?? $defaultTitle) }}"
               class="w-full border rounded px-3 py-2 focus:outline-none focus:ring" />
      </div>

      <div>
        <label class="block text-sm text-gray-600 mb-1">Subjudul (opsional)</label>
        <input type="text" name="subtitle" value="{{ old('subtitle', $customSection->subtitle ?? $defaultSubtitle) }}"
               class="w-full border rounded px-3 py-2 focus:outline-none focus:ring" />
      </div>
...
(function(){
  ...
  let i = 0;
  addBtn.addEventListener('click', ()=> itemsEl.appendChild(row(i++)));

  // render data lama
  const old = @json(old('items', ($customSection->payload['items'] ?? [])));
  if (old.length) {
    old.forEach(d => itemsEl.appendChild(row(i++, d)));
  } else {
    addBtn.click();
  }
})();
